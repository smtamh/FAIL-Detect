...
def get_all_raw_signals():
    ...
    PCA_kmeans_metric.extend([list(map(func_to_float, value[1].split('/'))) for key, value in eval_log.items() if cond_func(key)])
    logpO_metric.extend([list(map(func_to_float, value[2].split('/'))) for key, value in eval_log.items() if cond_func(key)])
    logpZO_metric.extend([list(map(func_to_float, value[3].split('/'))) for key, value in eval_log.items() if cond_func(key)])
    # DER_metric.extend([list(map(func_to_float, value[5].split('/'))) for key, value in eval_log.items() if cond_func(key)])
    NatPN_metric.extend([list(map(func_to_float, value[4].split('/'))) for key, value in eval_log.items() if cond_func(key)])
    CFM_metric.extend([list(map(func_to_float, value[5].split('/'))) for key, value in eval_log.items() if cond_func(key)])
    RND_metric.extend([list(map(func_to_float, value[6].split('/'))) for key, value in eval_log.items() if cond_func(key)])
    baselines = [PCA_kmeans_metric, logpO_metric, logpZO_metric, 
                 NatPN_metric, CFM_metric, RND_metric]                 # deleted DER and STAC: training failed in DER, STAC takes too much time in evaluation
...
for modify in [False, True]:
    for type_orig in ['square', 'transport', 'tool_hang', 'can']:
        ...
        PCA_kmeans_metric, logpO_metric, logpZO_metric, NatPN_metric, CFM_metric, RND_metric = everything[1:]
...
methods = ['PCA-kmeans', 'logpO', 'logpZO-Ot', 'NatPN', 'CFM', 'RND-Ot+At'] # deleted DER and STAC: training failed in DER, STAC takes too much time in evaluation
